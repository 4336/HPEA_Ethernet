...
void UART_TX(UART_Vars_t &u, MIT_CAN &m, Pressure_t &p, Flag_t &f, Command_t &c){
    ...
    uint16_t stamp = millis() & 0xFFFF;
    memcpy(u.tx_buf + 1, &stamp, 2);
    ...
    Serial.write(u.tx_buf, UART_TX_LEN);
#else //ASCII
    if(f.debug_timer && f.debug){
        f.debug_timer = false;
        Serial.print(millis()); Serial.print(' ');
        Serial.print(f.state); Serial.print(' ');
        Serial.print(m.get_pos(), 3); Serial.print(' ');
...
bool UART_RX(UART_Vars_t &u){
    ...
    }else if(Serial.available()>=UART_RX_LEN-1){
        for(int i=1; i<UART_RX_LEN; i++){
            u.rx_buf[i] = Serial.read();
        }
        uint16_t crc_get, crc_make;
        memcpy(&crc_get, u.rx_buf+UART_RX_LEN-3, 2);
        crc_make = MakeCRC16(u.rx_buf, UART_RX_LEN-3);
        if(crc_get == crc_make && u.rx_buf[UART_RX_LEN-1] == ETX){
            u.index = 0;
            return true;
        }else{
